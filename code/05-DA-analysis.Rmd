...
saveRDS(ps, here('data', 'following_study', 'ps_subset.rds'))
ps
```
...
saveRDS(ps.rare, here('data', 'following_study', 'ps_subset_rarefied.rds'))
ps.rare
```
...
test.results %>%
    ...
    filter(taxon %in% head(top.p$taxon, 20)) %>%
    ggplot() +
...
effect.plot2 <- test.results %>%
    filter(method != 'corncob') %>%
    mutate(direction = if_else(LFC > 0, 'positive', 'negative')) %>% 
    filter(taxon %in% head(top.p$taxon, 10)) %>%
    ggplot() +
    geom_point(aes(x = method, y = taxon, colour = direction, size = abs(LFC))) +
    theme(axis.text.x = element_text(angle = 60, hjust = 1))
...
cor.plot + effect.plot2 + plot_annotation(tag_levels = 'a')
ggsave(here('figures', 'DA-analysis2.png'), width = 8, height = 4.5)
```

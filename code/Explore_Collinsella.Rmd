...
sam$Household <- factor(sam$Household, levels = str_rank(unique(sam$Household), numeric = TRUE))
...
ggplot(otu.diff) +
    geom_bar(aes(x = Household, y = LFC, fill = ASV), stat = 'identity') +
    labs(y = 'log 2 fold change')

ggplot(otu.diff) +
    geom_bar(aes(x = Household, y = LFC, fill = (LFC > 0)), stat = 'identity') +
    facet_wrap(~ASV, scales = 'free_y') +
    theme(axis.text.x = element_blank()) +
    labs(y = 'log 2 fold change', fill = 'LFC > 0')

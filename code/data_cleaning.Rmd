...
```{r}
state.region <- usa::states %>% 
    select(name,region) %>%
    column_to_rownames('name') %>% 
    mutate(region = as.character(region))
meta_data <- meta_data %>%
    cbind(region = state.region[meta_data$State,])
```
...
```{bash, eval = FALSE}
raxml-ng --msa data/following_study/msa.fasta --model GTR+G --prefix data/following_study/tree
```

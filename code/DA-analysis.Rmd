...
```{r}
clr.data <- aldex.clr(t(otu_table(ps)), sample_data(ps)$Epileptic.or.Control, mc.samples=128, denom="all")
aldex.t.test <- data.frame(aldex.ttest(clr.data, paired.test = TRUE), aldex.effect(clr.data, paired.test = TRUE)) %>% 
    rownames_to_column('taxon') %>% 
    dplyr::select(taxon,diff.btw, we.ep) 

colnames(aldex.t.test)[2:3] <- c('LFC', 'p.value')
...
aldex.glm.test <- data.frame(aldex.glm(clr.data, model), aldex.glm.effect(clr.data, verbose = FALSE)) %>% 
    rownames_to_column('taxon') %>% 
    dplyr::select(taxon, `Epileptic.or.ControlEpileptic.diff.btw`, `Epileptic.or.ControlEpileptic.pval`)
colnames(aldex.glm.test)[2:3] <- c('LFC', 'p.value')
...
ancombc.test <- data.frame(taxon = ancombc.test$res$lfc$taxon,
                           LFC = ancombc.test$res$lfc$Epileptic.or.ControlEpileptic,
...
tb <- top.p %>% 
    ...
    data_color(columns = colnames(p.value.tb)[2:7],
               colors = scales::col_bin(palette = c(scales::hue_pal()(1), "transparent"),
                                        bins = c(-Inf, 0.05, Inf)))
tb
# gtsave(tb, file = here('figures','top10_p.png'))
```
...
test.results$taxon <- factor(test.results$taxon,
                             levels = str_sort(unique(test.results$taxon),
                                               numeric = TRUE))

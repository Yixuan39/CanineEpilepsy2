...
```{r message=FALSE}
aldex.tt <- aldex(otu_table(ps) %>% t, sam$Pheno.or.drug.naive,
...
```{r message=FALSE}
mm <- model.matrix( ~ Group + Pheno.or.drug.naive, sam)
...
```{r}
sig <- glm.test$`Pheno.or.drug.naivepheno:pval` < 0.1
aldex.plot(glm.effect[["Pheno.or.drug.naivepheno"]], test="effect")
points(glm.effect[["Pheno.or.drug.naivepheno"]]$diff.win[sig],
    glm.effect[["Pheno.or.drug.naivepheno"]]$diff.btw[sig], col="blue")
glm.test[sig,] %>%

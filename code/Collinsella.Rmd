...
sam.long <- sam %>%
    ...
    pivot_longer(cols = all_of(tax.collin.keep), names_to = 'ASV', values_to = 'abundance') %>% 
...
otu.diff <- data.frame(otu.epi - otu.ctl) %>%
    ...
    pivot_longer(all_of(tax.collin.keep), names_to = 'ASV', values_to = "LFC")
...
sapply(tax.collin.keep, function(taxa) pairwise_sign_test(filter(sam.long, ASV == taxa),
                                                          abundance ~ Epileptic.or.Control, 
                                                          ref.group = 'Epileptic', 
...
sam.long %>% 
    split(.$ASV) %>% 
    lapply(function(df) summary(xtabs(exist ~ Sex  + Epileptic.or.Control, data = df)))
```

```{r}
ftable(xtabs(exist ~ Sex  + Pheno.Y.N + ASV, data = sam.long))
```
...
sam.long %>% 
    ...
    split(.$ASV) %>% 
    lapply(function(df) chisq.test(xtabs(exist ~ Sex  + Pheno.Y.N, data = df)))
```
...
sam.long %>%
    split(.$ASV) %>%
    lapply(function(df) MASS::glm.nb((abundance + 1) ~ Age..months., data = df) %>% tidy())
```

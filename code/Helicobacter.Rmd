...
ps.helicobacter <- readRDS(here('data','following_study','ps_rarefied.rds')) %>%
    subset_taxa(Genus == 'Helicobacter') 
taxa_names(ps.genus) <- 'Genus'

# add genus level collinsella count to otu table
otu.df <- otu_table(ps.helicobacter) %>% 
    cbind(Genus = otu_table(ps.genus)) %>%
    cbind(data.frame(sample_data(ps.helicobacter))) %>% 
...
```{r message=FALSE}
helicobacter.prop <- readRDS(here('data','following_study','ps_rarefied.rds')) %>%
    transform_sample_counts(function(x) x / sum(x)) %>% 
    subset_taxa(Genus == 'Helicobacter') %>% 
    psmelt()
helicobacter.prop$OTU <- factor(helicobacter.prop$OTU, levels = str_sort(unique(helicobacter.prop$OTU), numeric = TRUE))
ggplot(helicobacter.prop) +

...
```{r}
get.similarity <- function(ps, paired.data, method) {
    dissimilarities <- if(method == 'jaccard') {
        phyloseq::distance(ps, method = method, binary = TRUE)
    } else {
        phyloseq::distance(ps, method = method)
    }
    dist <-  as.matrix(1 - dissimilarities)
    same.household.similarity <- sapply(1:nrow(paired.data[[1]]), function(i) {dist[paired.data[[1]][i,'Sample'], paired.data[[2]][i,'Sample']]})
    diff.household.similarity <- sapply(1:nrow(paired.data[[1]]), function(i) {dist[paired.data[[1]][i,'Sample'], paired.data[[3]][i,'Sample']]})
    data.frame(pair1, method, same.household.similarity, diff.household.similarity)
}

household.similarity <- map_dfr(c('jaccard', 'bray', 'wunifrac'), function(method) get.similarity(ps.rarefied, paired.data, method))
```
...
```{r}
household.similarity$method <- factor(household.similarity$method, levels = c('jaccard', 'bray', 'wunifrac'))
ggplot(household.similarity) +
    geom_point(aes(x = diff.household.similarity, y = same.household.similarity)) +
    geom_abline(intercept = 0, slope = 1, color = 'blue') +
    facet_wrap(~method) +
    labs(x = 'Similarity Between Different Household', y = 'Similarity Between Same Household')

household.similarity %>% 
  pivot_longer(c('same.household.similarity', 'diff.household.similarity'), names_to = 'type', values_to = 'similarity') %>%
  ggplot() +
      geom_boxplot(aes(x = type, y = similarity)) +
...
household.similarity %>% 
    split(.$method) %>%
    lapply(function(x) t.test(x$same.household.similarity,
                              x$diff.household.similarity, 
                              paired = TRUE,
                              alternative = 'greater'))

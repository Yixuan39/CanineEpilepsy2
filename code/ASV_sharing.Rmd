...
ps.same.depth <- rarefy_even_depth(ps,
                        sample.size = 40000, 
                        verbose = FALSE) # number of sample lost?
# ps.same.depth <- ps.same.depth %>%
#     transform_sample_counts(function(x) x / sum(x) )
```

Now, we have 106 samples.

```{r}
sam <- sample_data(ps.same.depth)
# following comments are how I find household to delete
# table(sam$Household)
# # which household has only one dog
# which(sam$Household %in% c(37,46))
# sam %>% data.frame %>% 
#     filter(Household %in% c(37,46))
# # household with more than two dogs
# sam %>% data.frame %>% 
#     filter(Household %in% c(17,18,26)) %>% 
#     arrange(Household) %>% 
#     select(SampleID2, Household, Epileptic.or.Control)
exclude <- c(39,79,111,112,49,50,37,38)
sub.ps <- ps.same.depth %>% 
    subset_samples(!(SampleID2 %in% exclude))
sub.sam <- sample_data(sub.ps) %>% data.frame()
```
...
otu <- otu_table(sub.ps)
# samples <- rownames(otu)
# this can calculate all possible comparison
# similarity <- outer(samples, samples,
#                     FUN = Vectorize(function(x, y) shared.ASV(x, y, otu)))
...
```{r}
unique.breed <- c()
for (house in unique(sub.sam$Household)) {
    # what is the proportion that same house has same breed
    same.house <- sub.sam %>% 
        filter(Household == house)
    other.house <- sub.sam %>% 
...
for(drop_n in 1:length(unique.breed)){
    ...
    test.sam <- sub.sam %>% filter(Household %!in% drop_house)
...
breed.control.sam <- sub.sam[-which(sub.sam$Household %in% drop_house_list[[drop]]),]
...
ggplot(breed.control.result) +
    ...
    tune::coord_obs_pred() +
    labs(x = 'Inter-group ASV Similarity',
         y = 'Intra-group ASV Similarity',
...
n_house <- unique(sub.sam$Household) %>% length()
Taxon <- tax_table(sub.ps) %>% 
    data.frame() %>% 
